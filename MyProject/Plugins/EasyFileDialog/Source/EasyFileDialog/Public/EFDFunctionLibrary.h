...
class EASYFILEDIALOG_API UEFDFunctionLibrary : public UBlueprintFunctionLibrary
{
    ...
    // Open Folder Dialog
    UFUNCTION(BlueprintCallable, Category = "EasyFileDialog")
        static const FString& OpenFolderDialog(const FString& DialogTitle, const FString& DefaultPath, FString& OutFolderName);
    UFUNCTION(BlueprintCallable, Category = "EasyFileDialog")
        static void Directorycopy(const FString& source, const FString& destination);
};
